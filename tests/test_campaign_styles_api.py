...
def test_types_route_hides_thrivemodal_objectives_from_an_arena_workspace(_keys):
    """The ThriveModal objectives share one AICB_CAMPAIGN_TYPES list with
    Arena's shapes, so the route has to gate them the way the in-app picker
    does. An Arena workspace (the default, and every workspace that predates
    the ThriveModal playbook) must never be offered a tm_* template."""
    key = fa._mint_api_key(_OWNER_A)
    r = _client().get("/api/v1/campaign_types", headers={"X-API-Key": key})
    assert r.status_code == 200
    keys = {t["key"] for t in r.json()}
    assert not (keys & fa._TM_TYPE_KEYS)


def test_types_route_on_a_thrivemodal_workspace_returns_only_its_allowlist(
        _keys, monkeypatch):
    """Mirror of the Arena case: a ThriveModal workspace sees its six
    objectives plus the two playbook-neutral shapes, and none of Arena's
    recruiting templates (whose step prompts carry claims that playbook
    forbids)."""
