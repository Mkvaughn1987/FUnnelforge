...
export default function CreateCampaignPage() {
  ...
  const [mode, setMode] = useState<"choose" | "manual" | "ai" | "saved">("choose");
  ...
  if (mode === "saved") {
    const savedCampaigns = [
      { id: "1", name: "Q1 Cold Outreach", type: "Cold Outreach", emails: 7, status: "Draft", updated: "Feb 18, 2026" },
      { id: "2", name: "Nurture 2025", type: "Nurture Sequence", emails: 5, status: "Active", updated: "Feb 15, 2026" },
      { id: "3", name: "Re-engagement Campaign", type: "Follow-Up", emails: 4, status: "Draft", updated: "Feb 10, 2026" },
    ];
    return (
      <div>
          ...
          {savedCampaigns.map((c) => (
            ...
            >
              ...
              <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                c.status === "Active" ? "bg-green-400/10 text-green-400" : "bg-accent-blue/10 text-accent-blue"
              }`}>
                {c.status}
              </span>
              <svg className="w-4 h-4 text-text-secondary group-hover:text-accent-blue transition-colors flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    ...
    return (
      <div>
        ...
        <div className="bg-surface rounded-xl border border-border p-6 space-y-6 max-w-2xl">
          ...
          <button
            onClick={() => router.push("/create-campaign/email-editor")}
            className="px-5 py-2.5 rounded-lg bg-accent-blue text-white text-sm font-medium hover:bg-accent-blue/90 transition-colors"
          >
