# funnelforge_gui.py
# Funnel Forge – GUI (UPDATED / POLISHED LAYOUT WITH SIDEBAR NAV)
# - Left sidebar navigation: Dashboard / Build emails / Sequence & schedule / Contacts / Preview / Test / Cancel pending
# - Each nav item swaps the entire main content area (no more single giant page)
# - "Build emails" screen is your full original layout:
#       * Email editor on the left (tabs, templates, variables, per-email attachments)
#       * Right side: Customize your sequence + Contact list + Tools (preview / run / cancel)
# - Other screens reuse the same underlying logic but focus the UI:
#       * Sequence & schedule: sequence table + Create sequence
#       * Contacts: contact list box
#       * Preview / Test: preview emails box
#       * Cancel pending: cancel pending box
#
# - Per-email attachments managed via one "Add / Delete" button per row
# - Templates + Variables panels swapped (Templates above Variables), both collapsed by default
# - Contacts workflow: Import -> converts to OFFICIAL contacts.csv (single source of truth)
# - Status bar with pulsing dot + verb-style messages
#
# NOTE: GUI-only file; calls funnelforge_core.run_4drip(...)
#       Core interprets date+time as local machine time.
...
# Backgrounds
BG_ROOT = "#F7F9FC"          # Light blue-gray app background
BG_CARD = "#FFFFFF"          # White cards
BG_SIDEBAR = "#FFFFFF"       # White sidebar
BG_HEADER = "#1E293B"        # Dark slate top banner
BG_ENTRY = "#FFFFFF"         # White input fields
BG_HOVER = "#F1F5F9"         # Slate-100 hover state

# Accent Colors
ACCENT = "#06B6D4"           # Cyan-500 (primary teal/aqua)
ACCENT_HOVER = "#0891B2"     # Cyan-600
ACCENT_LIGHT = "#E0F2FE"     # Cyan-50
DARK_AQUA = "#06B6D4"        # Primary button color
DARK_AQUA_HOVER = "#0891B2"  # Primary button hover
...
# Text Colors
FG_TEXT = "#1E293B"          # Dark slate (primary text)
FG_MUTED = "#64748B"         # Slate-500 (secondary text)
FG_LIGHT = "#94A3B8"         # Slate-400 (muted text)
FG_WHITE = "#FFFFFF"         # White text (on dark backgrounds)

# Borders
BORDER = "#E2E8F0"           # Slate-200 (light border)
BORDER_MEDIUM = "#CBD5E1"    # Slate-300 (medium border)
BORDER_SOFT = "#F1F5F9"      # Slate-100 (very light)
ACCENT_2 = "#E2E8F0"         # Border accent
...
# Card/Panel spacing
CARD_OUTER_PAD = 16    # Space around cards in main area
CARD_INNER_PAD = 16    # Space inside cards
SECTION_SPACING = 16   # Space between sections within a card
FIELD_SPACING = 12     # Space between form fields
...
def detect_and_convert_contacts_to_official(src_csv: str, dest_csv: str) -> Tuple[int, List[str]]:
    ...
    if written == 0:
        warnings.append("No valid contacts written (Email missing/invalid).")
...
class FunnelForgeApp(tk.Tk):
    ...
    def _build_header(self):
        """Build modern header with dark background"""
        ...
        right_section = tk.Frame(header, bg=BG_HEADER)
        right_section.pack(side="right", padx=24)
    ...
    def _build_nav_and_content(self):
        ...
        try:
            ...
            print("✅ Sequence screen created successfully")
        except Exception as e:
        ...
        try:
            ...
            print("✅ Contacts screen created successfully")
        except Exception as e:
        ...
        try:
            ...
            print("✅ Execute screen created successfully")
        except Exception as e:
    ...
    def _refresh_tab_labels(self):
        """Refresh all email tab labels with X delete buttons (hide X if only 1 email)"""
        if not hasattr(self, "email_notebook"):
            return
    ...
    def _build_email_editor(self, parent):
        ...
        btn_bar = tk.Frame(card, bg=BG_CARD)
        btn_bar.pack(fill="x", padx=10, pady=(10, 0))

        # Green + Add Email button (left side)
        tk.Button(
            btn_bar,
            text="+ Add Email",
        ...
        # Bind left-click to handle delete button (X) clicks on tabs
        self.email_notebook.bind("<Button-1>", self._on_tab_click)

        # Button bar for template buttons (right side)
        btn_bar_templates = tk.Frame(card, bg=BG_CARD)
        btn_bar_templates.pack(fill="x", padx=10, pady=(10, 0))

        # Template buttons (right side)
        tk.Button(
            btn_bar_templates,
        ...
        tk.Button(
            ...
            command=self._upload_email_template,
            bg="#F1F5F9",
            fg=FG_TEXT,
            activebackground=BG_HOVER,
            activeforeground=FG_TEXT,
        ...
        tk.Button(
            ...
            command=self._create_template_from_current,
            bg="#F1F5F9",
            fg=FG_TEXT,
            activebackground=BG_HOVER,
            activeforeground=FG_TEXT,
        ...
        self.email_notebook.pack(fill="both", expand=True, padx=10, pady=10)
    ...
    def _create_email_tab(self, index: int, name_var: tk.StringVar, subject_var: tk.StringVar, body_text: str = "") -> tk.Text:
        ...
        # Add tab (X will be shown/hidden by _refresh_tab_labels)
        label = name_var.get().strip() or f"Email {index}"
        self.email_notebook.add(tab, text=f"  {label}  ")

        _, inner = self._make_fancy_box(
            tab,
            title="Build your email campaign",
            subtitle="Create up to 15 separate emails in one campaign",
        )

        tk.Label(inner, text="Name", bg=BG_CARD, fg=ACCENT, font=FONT_FIELD_HDR).pack(anchor="w")
        ent_name = self._styled_entry(inner, name_var)
        ent_name.pack(fill="x", pady=(6, 12))

        def _on_name_change(*_):
            self._refresh_tab_labels()
        name_var.trace_add("write", _on_name_change)

        # Date and Time fields (right under Name)
        datetime_row = tk.Frame(inner, bg=BG_CARD)
        datetime_row.pack(fill="x", pady=(0, 12))

        # Date field (left side) - using DateEntry like Set Schedule
        date_frame = tk.Frame(datetime_row, bg=BG_CARD)
        date_frame.pack(side="left", fill="x", expand=True, padx=(0, 8))

        tk.Label(date_frame, text="Send Date", bg=BG_CARD, fg=ACCENT, font=FONT_FIELD_HDR).pack(anchor="w")
        date_widget = self._dateentry_widget(date_frame, self.date_vars[index-1])
        date_widget.pack(fill="x", pady=(6, 0))

        # Time field (right side)
        time_frame = tk.Frame(datetime_row, bg=BG_CARD)
        time_frame.pack(side="left", fill="x", expand=True)

        tk.Label(time_frame, text="Send Time", bg=BG_CARD, fg=ACCENT, font=FONT_FIELD_HDR).pack(anchor="w")
        time_combo = ttk.Combobox(
            time_frame,
            textvariable=self.time_vars[index-1],
            values=TIME_OPTIONS,
            state="readonly",
            style="Dark.TCombobox",
            font=FONT_BASE
        )
        time_combo.pack(fill="x", pady=(6, 0))
        
        # Note: No need to rebuild sequence table on date/time changes
        # The sequence table widgets are already bound to these StringVars
        # and will update automatically


        tk.Label(inner, text="Subject", bg=BG_CARD, fg=ACCENT, font=FONT_FIELD_HDR).pack(anchor="w")
        self._styled_entry(inner, subject_var).pack(fill="x", pady=(6, 0))

        # Variables panel (Templates panel removed per user request)
        self._create_variables_panel(inner)

        tk.Label(inner, text="Body", bg=BG_CARD, fg=ACCENT, font=FONT_FIELD_HDR).pack(anchor="w")
        txt_body = tk.Text(
            inner,
            bg=BG_ENTRY,
            fg=FG_TEXT,
        ...
        txt_body.pack(fill="both", expand=True, pady=(6, 0))
    ...
    def _delete_email(self, index: int):
        ...
        # Update sequence table and tab labels
        self._rebuild_sequence_table()
        self._refresh_tab_labels()
