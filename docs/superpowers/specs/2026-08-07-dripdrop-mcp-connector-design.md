...
for the consent screen: the existing email/password `login_page`
(`flowdrip_app.py:53370`), which sets `app.storage.user` on success.
...
official MCP Python SDK's `mcp.server.fastmcp.FastMCP`, which — given an
`auth_server_provider` implementing `OAuthAuthorizationServerProvider` —
...
`load_refresh_token`, `exchange_refresh_token`, `load_access_token`) and the
three tools.
...
**Auth flow:**
1. Claude hits `/authorize` with a PKCE challenge. If the caller doesn't have
   an active DripDrop session, the provider redirects to DripDrop's existing
   `/login?next=...`, stashing the pending OAuth request (client_id, redirect_uri,
   code_challenge, state) in a short-lived server-side record keyed by a nonce
   in the `next` URL.
2. User logs in with their normal DripDrop email/password (same login they
   already use — no new credential system).
...
to `http://localhost:8080/api/v1/...` with `X-API-Key` set to that key. Key
resolution happens fresh on every call rather than being baked into the OAuth
token, so:
...
- A user who has never generated a DripDrop API key gets a clear tool-error
  ("No DripDrop API key on file — generate one at Profile → Personal Info →
  API Access, then reconnect") instead of a silent failure.

Calling over `localhost` sidesteps the Cloudflare WAF same-origin check
entirely (see `dripdrop-local-session-api-headers` memory) — no need to fake
...
| `check_candidate_count` | `GET /api/v1/candidates/count` | Read-only sanity/dry-run check — `{active, placed, on_hold, total}`. Confirms the connector is authorized before a real send. |
...
`https://mcp.dripdripdrop.ai/mcp` → redirected to DripDrop login → log in
(existing DripDrop account) → connected. From then on, both interactive and
...
- Manual end-to-end: run the server locally against local DripDrop
  ...
  complete the login redirect, call `check_candidate_count` (safe, read-only)
